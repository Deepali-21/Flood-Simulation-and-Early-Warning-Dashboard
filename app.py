...
RISK_COLORS = {0: "#2ecc71", 1: "#f1c40f", 2: "#e74c3c"}
...
def build_heatmap_figure(result: dict, view: str, cls: np.ndarray = None) -> go.Figure:
    ...
    frames = [
        go.Frame(
            data=[go.Heatmap(z=z_stack[i], colorscale=colorscale, zmin=0, zmax=zmax,
    ...
    fig = go.Figure(
        data=[go.Heatmap(z=z_stack[0], colorscale=colorscale, zmin=0, zmax=zmax,
                          colorbar=dict(title=colorbar_title))],
        frames=frames,
    )
...
st.title("🌊 FloodShield — Flood Simulation & Early Warning Dashboard")
...
with tab_compare:
    ...
    if st.button("Compare"):
        ...
        line_fig.update_layout(title="Average city-wide depth over time",
        ...
        with map_col1:
            st.plotly_chart(
                go.Figure(go.Heatmap(z=summary_a["classification"][-1],
                                      colorscale=[[0, RISK_COLORS[0]], [0.5, RISK_COLORS[1]], [1, RISK_COLORS[2]]],
                                      zmin=0, zmax=2))
                .update_layout(title=f"{scen_a} — final risk", height=400,
                                yaxis=dict(autorange="reversed")),
                use_container_width=True,
            )
        with map_col2:
            st.plotly_chart(
                go.Figure(go.Heatmap(z=summary_b["classification"][-1],
                                      colorscale=[[0, RISK_COLORS[0]], [0.5, RISK_COLORS[1]], [1, RISK_COLORS[2]]],
                                      zmin=0, zmax=2))
                .update_layout(title=f"{scen_b} — final risk", height=400,
                                yaxis=dict(autorange="reversed")),
                use_container_width=True,
            )
